    ...
    <div dir="rtl" className="space-y-4 p-5 md:p-7 rounded-3xl border border-border bg-card shadow-sm font-arabic-body text-right">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-2xl border border-border bg-background pr-10 pl-4 py-2.5 text-xs font-bold text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
        />
      </div>
        ...
        {filteredItems.length === 0 ? (
          ...
          filteredItems.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="rounded-2xl border border-border/60 bg-secondary/20 p-4 md:p-5 space-y-3 hover:border-primary/30 transition-all shadow-sm"
              >
                ...
                {/* Key Difference Note */}
                <div className="text-xs text-muted-foreground bg-primary/10 border border-primary/20 p-3 rounded-xl flex items-start gap-2">
                  <span className="font-bold text-primary shrink-0">الفارق اللفظي الدقيق:</span>
